...
<template>
  <header class="topbar">
    ...
    <div class="topbar-actions"><span v-if="currentMember" class="member-greeting"><MemberAvatar :name="currentMember.name" size="small" />Bonjour {{ currentMember.name }}</span><button type="button" class="logout-button" @click="logout">Se déconnecter</button></div>
  </header>
  ...
  <main class="page-shell">
    ...
    <div v-if="warningTask" class="confirmation-overlay" @click.self="dismissWarning"><section class="confirmation-dialog deadline-dialog"><img class="warning-image" src="https://media1.tenor.com/m/4NGmv1oM-dUAAAAC/p5-p5r.gif" alt="Avertissement d’échéance" /><p class="confirmation-kicker">TIME IS RUNNING OUT</p><h2>À faire bientôt</h2><p class="confirmation-message">« <strong>{{ warningTask.title }}</strong> » doit être terminée avant {{ warningTask.deadline }}.</p><div class="confirmation-actions"><button type="button" class="confirm-button" @click="dismissWarning">J'ai compris</button></div></section></div>
